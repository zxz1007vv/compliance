...
class PPO_Args(PrefixProto, cli=False):
    # algorithm
    value_loss_coef = 1.0
...
class PPO:
    ...
    def __init__(self, actor_critic, device='cpu', cfg=None):

        self.device = device
        self.cfg = cfg if cfg is not None else PPO_Args

        # PPO components
        self.actor_critic = actor_critic
        self.actor_critic.to(device)

        self.cfg.adaptation_labels = self.actor_critic.adaptation_labels
        self.cfg.adaptation_dims = self.actor_critic.adaptation_dims
        self.cfg.adaptation_weights = self.actor_critic.adaptation_weights
    ...
    def update(self):
        ...
        label_start_end = {}
        si = 0
        for idx, (label, length) in enumerate(zip(self.cfg.adaptation_labels, self.cfg.adaptation_dims)):
        ...
        for obs_batch, critic_obs_batch, privileged_obs_batch, obs_history_batch, actions_batch, target_values_batch, advantages_batch, returns_batch, old_actions_log_prob_batch, \
            ...
            if len(self.cfg.adaptation_labels) > 0:
                ...
                for epoch in range(self.cfg.num_adaptation_module_substeps):
                    ...
                    adaptation_loss = 0
                    for idx, (label, length, weight) in enumerate(zip(self.cfg.adaptation_labels, self.cfg.adaptation_dims, self.cfg.adaptation_weights)):
        ...
        mean_adaptation_module_loss /= (num_updates * self.cfg.num_adaptation_module_substeps * self.cfg.adaptation_batch_size)
        ...
        for label in self.cfg.adaptation_labels:
            mean_adaptation_losses[label] /= (num_updates * self.cfg.num_adaptation_module_substeps * self.cfg.adaptation_batch_size)
        self.storage.clear()
