...
Yaw 状态行及分段汇总给出实际底盘 yaw rate、左右轮平均滚动线速度和
`wheel_yaw_hat`。当前部署合同中，只要命令含 yaw，控制器就会记录进入该模式时的轮角，
并使用与训练端一致的 PD 增益锁住四个轮子；因此轮速和 `wheel_yaw_hat` 应接近零，实际
yaw 应由对角踏步产生。`pure_x` 才继续使用轮 action 驱动。
完整参数可运行：
