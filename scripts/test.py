...
def test(headless: bool = False):
    ...
    # Explicitly release Isaac Gym's native resources before Python teardown.
    cameras.clear()
    env.close()


if __name__ == '__main__':
    # Set headless=False only when a working graphical display is available.
    test(headless = True)
    # Isaac Gym Preview 4 can segfault while its native libraries are unloaded.
    # The simulation has already been closed explicitly by test().
    sys.stdout.flush()
    sys.stderr.flush()
    os._exit(0)
