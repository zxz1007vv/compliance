...
from utils import resolve_local_checkpoint
...
def train_b1_z1_IK(headless=True, max_iterations=None, save_interval=None,
                   training_name=None, resume_run_dir=None, checkpoint="latest", **deps):
    ...
    RunnerArgs.save_video_interval = 0   #关闭视频保存
    if max_iterations is not None:
        RunnerArgs.max_iterations = max_iterations
    if save_interval is not None:
        RunnerArgs.save_interval = save_interval
    if training_name is not None:
        RunCfg.training_name = training_name
    ...
    # TensorBoard is used by default. Set COMPLIANCE_LOGGER=wandb or both to
    # explicitly enable the optional W&B backend.
    log_config = {
        "RunCfg": vars(RunCfg),
    ...
    runner = Runner(
        env,
        device=sim_device,
        task_name=RunCfg.task_name,
        run_name=RunCfg.training_name,
    ...
    if resume_run_dir is not None:
        _, checkpoint_path, _ = resolve_local_checkpoint(resume_run_dir, checkpoint)
        runner.load(checkpoint_path)
        print(f"Resuming training from: {checkpoint_path}")
    runner.learn(
        num_learning_iterations=RunnerArgs.max_iterations,
