...
class VelocityTrackingEnv(LeggedRobot):
    ...
    def step(self, actions):
        ...
        # These large CPU arrays are only needed by evaluation/play scripts. Copying
        # them for thousands of training environments forces a CUDA sync every step.
        if self.cfg.env.send_eval_data:
            self.extras.update({
                "joint_pos": self.dof_pos.cpu().numpy(),
                "joint_vel": self.dof_vel.cpu().numpy(),
                "joint_pos_target": self.joint_pos_target.detach().cpu().numpy(),
                "joint_vel_target": torch.zeros(12),
                "body_linear_vel": self.base_lin_vel.detach().cpu().numpy(),
                "body_angular_vel": self.base_ang_vel.detach().cpu().numpy(),
                "body_linear_vel_cmd": self.commands[:, 0:2].cpu().numpy(),
                "body_angular_vel_cmd": self.commands[:, 2:].cpu().numpy(),
