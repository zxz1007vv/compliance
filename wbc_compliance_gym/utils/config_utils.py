...
def normalize_saved_env_config(default_config, saved_config):
    ...
    This also migrates asset paths saved before the combined B1+Z1 resource
    directory was renamed from ``resources/robots/b1`` to
    ...
    coefficients without changing valid current configs.
    ...
    if isinstance(asset, dict) and isinstance(asset.get("file"), str):
        ...
        new_path = old_path.replace(
            "/resources/robots/b1/", "/resources/robots/b1_z1/"
        )
        if new_path != old_path:
            asset["file"] = new_path
            report["legacy_asset_path_migrated"] = True
            report["asset_path_before"] = old_path
            report["asset_path_after"] = new_path
